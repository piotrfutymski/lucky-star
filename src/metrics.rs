...
impl MetricsCache {
    ...
    pub fn load(path: &Path, fingerprint: &str) -> Result<Self, String> {
        ...
        Ok(if cache.is_compatible(fingerprint) { cache } else { Self::empty(fingerprint) })
    }
    ...
    pub fn upsert(&mut self, record: CacheRecord) {
        if let Some(old) = self.records.iter_mut().find(|r| r.file_name == record.file_name) {
            *old = record;
        } else {
...
pub fn record_for(path: &Path, fingerprint: impl Into<String>, metrics: MetricValues) -> Result<CacheRecord, String> {
    let meta = fs::metadata(path).map_err(|e| e.to_string())?;
    Ok(CacheRecord {
        file_name: path.file_name().and_then(|n| n.to_str()).ok_or("invalid file name")?.to_string(),
        size: meta.len(),
...
impl Metric {
    ...
    pub fn requires_pattern(self) -> bool {
        matches!(self, Self::QualityStarPattern | Self::StarBrightness | Self::Snr)
    }
...
impl FilterRule {
    pub fn relative(metric: Metric, multiplier: f64) -> Result<Self, String> {
        if !multiplier.is_finite() || multiplier <= 0.0 { return Err("relative threshold must be greater than zero".into()); }
        Ok(Self { metric, relative: Some(multiplier), absolute: None })
    }

    pub fn absolute(metric: Metric, threshold: f64) -> Result<Self, String> {
        if !threshold.is_finite() { return Err("absolute threshold must be finite".into()); }
        Ok(Self { metric, relative: None, absolute: Some(threshold) })
    }

    pub fn threshold(&self, median: Option<f64>) -> Result<f64, String> {
        match (self.relative, self.absolute) {
            (Some(_), Some(_)) => Err(format!("both relative and absolute thresholds supplied for {}", self.metric.key())),
            (Some(factor), None) => median.map(|m| factor * m).ok_or_else(|| format!("no values available for {}", self.metric.key())),
...
pub fn medians(records: &[CacheRecord]) -> BTreeMap<Metric, f64> {
    ...
    for metric in [Metric::Quality, Metric::Fwhm, Metric::QualityStarPattern, Metric::Background, Metric::StarBrightness, Metric::Snr] {
        let mut values: Vec<f64> = records.iter().filter_map(|r| metric.value(&r.metrics)).filter(|v| v.is_finite()).collect();
        if let Some(m) = median(&mut values) { result.insert(metric, m); }
    }
    result
}

pub fn passes_all_filters(record: &CacheRecord, rules: &[FilterRule], medians: &BTreeMap<Metric, f64>) -> Result<bool, String> {
    for rule in rules {
        let Some(value) = rule.metric.value(&record.metrics) else { return Ok(false) };
        let threshold = rule.threshold(medians.get(&rule.metric).copied())?;
        let passes = if rule.metric.higher_is_better() { value >= threshold } else { value <= threshold };
...
pub fn unique_removed_folder(dir: &Path, rules: &[FilterRule], timestamp: u64) -> PathBuf {
    let base = if rules.len() == 1 {
        let r = &rules[0];
        let suffix = r.relative.map(|v| format!("{v:.3}")).or_else(|| r.absolute.map(|v| format!("{v:.3}"))).unwrap_or_default();
        dir.join(format!("removed_{}_{}", r.metric.key(), suffix))
    } else { dir.join(format!("removed_combined_{timestamp}")) };
...
mod tests {
    ...
    fn values(quality: f64, snr: Option<f64>) -> MetricValues {
        MetricValues { quality, fwhm: 2.0, star_count: 3, brightest_star_adu: 10.0,
            background_raw_adu: 20.0, background_corrected_adu: 20.0,
            quality_star_pattern: snr, star_brightness_adu: snr.map(|x| x * 10.0), snr,
            star_pattern_found: snr.is_some(), matched_star_count: usize::from(snr.is_some()) }
    }

    fn record(name: &str, quality: f64, snr: Option<f64>) -> CacheRecord {
        CacheRecord { file_name: name.into(), size: 1, modified_ns: 1,
            cache_format_version: CACHE_FORMAT_VERSION, algorithm_version: ALGORITHM_VERSION.into(),
            configuration_fingerprint: "config-a".into(), metrics: values(quality, snr) }
    }
    ...
    fn cache_round_trip_and_stale_configuration_invalidates_everything() {
        ...
        assert_eq!(MetricsCache::load(&path, "config-a").unwrap().records.len(), 1);
        assert!(MetricsCache::load(&path, "config-b").unwrap().records.is_empty());
        let _ = fs::remove_dir_all(dir);
    }

    #[test]
    fn relative_rules_use_median_and_are_and_combined() {
        let records = vec![record("a", 1.0, Some(10.0)), record("b", 0.4, Some(20.0)), record("c", 0.6, None)];
    ...
    fn absolute_rules_obey_metric_direction_and_conflicts_are_rejected() {
        ...
        assert!(passes_all_filters(&r, &[FilterRule::absolute(Metric::Fwhm, 2.0).unwrap()], &med).unwrap());
        assert!(!passes_all_filters(&r, &[FilterRule::absolute(Metric::Background, 1.0).unwrap()], &med).unwrap());
    ...
    fn cache_records_keep_missing_pattern_metrics_as_missing() {
        ...
        assert!(!passes_all_filters(&r, &[FilterRule::absolute(Metric::Snr, 1.0).unwrap()], &BTreeMap::new()).unwrap());
    }
